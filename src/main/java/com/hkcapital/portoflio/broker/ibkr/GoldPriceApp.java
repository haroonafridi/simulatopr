package com.hkcapital.portoflio.broker.ibkr;
...
public class GoldPriceApp implements EWrapper {

    private EJavaSignal m_signal = new EJavaSignal();
    private EClientSocket m_client = new EClientSocket(this, m_signal);

    public static void main(String[] args) {
        GoldPriceApp app = new GoldPriceApp();
        // Use 7497 for TWS Paper Trading or 4002 for Gateway Paper Trading
        app.m_client.eConnect("127.0.0.1", 7497, 0);

        // Start a thread to process messages from IBKR
        final EReader reader = new EReader(app.m_client, app.m_signal);
        reader.start();
        new Thread(() -> {
            while (app.m_client.isConnected()) {
                app.m_signal.waitForSignal();
                try {
                    reader.processMsgs();
                } catch (Exception e) {
    ...
    public  void placeOrder() {
        ...
        order.totalQuantity(Decimal.get(4));
        m_client.placeOrder(1003,contract,order);
    }

    @Override
    public void nextValidId(int orderId) {
        System.out.println("Connected. Requesting Gold Data...");
        Contract gold = new Contract();
        gold.symbol("XAUUSD");
        gold.secType("CMDTY");   // Important: Commodity type
        gold.exchange("SMART"); // IBKR's metals/forex network
        gold.currency("USD");
        // RequestId 1001 for tracking this specific data stream
        m_client.reqMktData(1001, gold, "", false, false, null);
    }
    ...
    public void tickPrice(int tickerId, int field, double price, TickAttrib attribs) {
        if (tickerId == 1001) {
            ...
            if (field == 1) System.out.println(">>> GOLD BID: " + price);
            if (field == 2) System.out.println(">>> GOLD ASK: " + price);
        }
    }

    // --- Mandatory EWrapper Overrides (Simplified for this example) ---
    ...
    @Override public void connectionClosed() { System.out.println("Connection Closed."); }
    ...
    // Remaining EWrapper methods can be left empty for a basic data fetcher
    ...
    @Override public void tickNews(int tickerId, long time, String providerCode, String articleId, String headline, String extraData) {}
